...
{
    /**
     * Menampilkan daftar alokasi honor dengan fitur pencarian dan filter tahun.
     */
    public function index(Request $request)
    {
        ...
        // Fallback jika relasi SPK tidak terhubung sempurna
    ...
    /**
     * API untuk mendapatkan info Honor Berjalan, Sisa SBML, dan HKS.
     */
    public function getHonorInfo(Request $request)
    {
        ...
        // HITUNG AKUMULASI POKOK saat ini
        ...
        // Cari Aturan SBML yang relevan
        $sbml = AturanSbml::where('id_jabatan', $request->id_jabatan)
            ->where('tahun_berlaku', $spk->tahun)
    ...
    /**
     * Menyimpan Alokasi Honor dengan Mitigasi Sisi Server
     */
    ...
    public function update(Request $request, $id)
    {
        ...
        // 2. Validasi SBML saat update
        $pokok_baru = (float)$request->harga_input * (float)$request->volume_target;
        
        // Hitung total honor lain (kecuali data yang sedang di-edit ini)
        ...
        $volume = $request->volume_target;
        $nilaiLain = $request->nilai_lain ?? 0;
        
        $alokasi->update([
            'harga_satuan_aktual' => $request->harga_input,
            'volume_target' => $volume,
            'nilai_lain' => $nilaiLain,
            'total_honor' => $pokok_baru + $nilaiLain,
        ]);
        ...
        $rekapData = $alokasiData->groupBy(function($item) {
            ...
            return $itemsByMitra->groupBy(function($item) {
                return ($item->spk && $item->spk->bulan && $item->spk->tahun) 
                    ? \Carbon\Carbon::createFromDate($item->spk->tahun, $item->spk->bulan, 1)->translatedFormat('F Y')
