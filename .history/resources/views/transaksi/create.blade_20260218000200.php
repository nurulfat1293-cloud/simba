...
<script>
    ...
    async function fetchHonorInfo() {
        ...
        try {
            // Panggil API BARU yang kita buat (ApiHelperController)
            ...
            if (data.status === 'success') {
                // Mapping hasil JSON dari Calculator ke UI kamu
                ...
                document.getElementById('nama_sbml_label').innerText = "Gagal Memuat";
            }

        } catch (error) { 
            console.error("Error fetching honor info:", error); 
            document.getElementById('nama_sbml_label').innerText = "Error Koneksi";
        }
    }
