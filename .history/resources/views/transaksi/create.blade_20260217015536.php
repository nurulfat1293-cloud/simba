                    ...
                    <div class="p-6 bg-slate-900 rounded-2xl text-white shadow-lg border-b-4 border-slate-700">
                        ...
                        <div class="space-y-4">
                            <div id="sbml_rule_info" class="bg-slate-800 p-3 rounded-lg border border-slate-700">
                                <p class="text-[10px] text-slate-400 uppercase font-bold mb-1">Kategori Aturan:</p>
...
<script>
    ...
    async function loadSpk(kegId, selectedSpk = null) {
        ...
        try {
            ...
            dataSpk.forEach(s => {
                ...
                idSpk.innerHTML += `<option value="${s.nomor_spk}" ${isSelected}>${s.nomor_spk} | ${namaMitra}</option>`;
            });
    ...
    async function fetchHonorInfo() {
        ...
        try {
            const url = `/api/get-honor-info?id_spk=${encodeURIComponent(spkId)}&id_kegiatan=${kegId}&id_jabatan=${jabId}`;
            const res = await fetch(url);
            const data = await res.json();
            
            document.getElementById('honor_spk_display').innerText = formatIDR(data.honor_diterima_spk || 0);
            document.getElementById('sbml_max_display').innerText = formatIDR(data.limit_sbml || 0);
            document.getElementById('sisa_sbml_display').innerText = formatIDR(data.sisa_sbml || 0);
            document.getElementById('nama_sbml_label').innerText = data.nama_sbml || 'Aturan Standar';
            
            document.getElementById('sisa_sbml_display').setAttribute('data-raw-sisa', data.sisa_sbml || 0);
            document.getElementById('sbml_max_display').setAttribute('data-raw-max', data.limit_sbml || 0);

        } catch (error) { 
            console.error("Error fetching honor info:", error); 
        }
    }
    ...
    function calculateHonor() {
        ...
        if (limitSbml > 0) {
            ...
            usedPercentage = ((alreadyUsed + totalInput) / limitSbml) * 100;
        }
        ...
        const isOverSbml = limitSbml > 0 && totalInput > (sisaKapasitas + 0.1); // margin kecil untuk float
