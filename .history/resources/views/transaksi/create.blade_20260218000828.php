...
<script>
    ...
    async function loadSpk(kegId, selectedSpk = null) {
        ...
        try {
            ...
            dataSpk.forEach(s => {
                ...
                // PERBAIKAN: Value option menggunakan s.id, bukan s.nomor_spk
                // Backend membutuhkan ID (Integer) untuk relasi, bukan String Nomor SPK
    ...
    // === PERBAIKAN UTAMA: LOGIC FETCH SBML DENGAN POST ===
    async function fetchHonorInfo() {
        ...
        try {
            // Gunakan API check-honor-limit (POST) bukan get-honor-info (GET)
            const response = await fetch("{{ route('api.check-honor-limit') }}", {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Accept': 'application/json', // Memastikan response error dari Laravel berupa JSON
            ...
            // Cek status HTTP dulu (misal 404, 500)
            if (!response.ok) {
            ...
            if (data.status === 'success') {
                ...
                document.getElementById('nama_sbml_label').innerText = "Gagal Memuat: " + (data.message || 'Unknown Error');
            }
    ...
    function calculateHonor() {
        ...
        // Cek kondisi over limit (dengan toleransi floating point)
