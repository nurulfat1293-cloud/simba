...
<script>
    ...
    async function loadSpk(kegId, selectedSpk = null) {
        ...
        try {
            ...
            dataSpk.forEach(s => {
                ...
                // PERBAIKAN: Gunakan s.nomor_spk agar value select tidak "undefined"
    ...
    async function fetchHonorInfo() {
        ...
        // Jangan request jika SPK belum dipilih atau nilainya kosong/undefined
        ...
        try {
            const response = await fetch("{{ route('api.check-honor-limit') }}", {
                ...
                body: JSON.stringify({
                    spk_id: spkId, // Ini akan mengirim string nomor_spk (cth: "SPK-001")
                    kegiatan_id: kegId,
                    jabatan_id: jabId
                })
            });
            ...
            if (data.status === 'success') {
                ...
                document.getElementById('nama_sbml_label').innerText = "Gagal: " + (data.message || 'Unknown');
            }
