<div class="space-y-6">
    ...
    <div class="flex items-center justify-between bg-slate-50 p-4 rounded-xl border border-slate-100">
        ...
        <div class="text-right">
            ...
            <p class="text-sm font-mono font-bold text-slate-700">#{{ str_pad($alokasi->id ?? 0, 5, '0', STR_PAD_LEFT) }}</p>
        </div>
    </div>
    ...
    <div class="space-y-5 px-1">
        <div class="grid grid-cols-2 gap-4">
            <div>
                ...
                <p class="text-sm font-semibold text-slate-800">
                    {{-- PERBAIKAN: Penanganan Null pada relasi bertingkat --}}
                    {{ optional(optional($alokasi->spk)->mitra)->nama_lengkap ?? 'Data Mitra Hilang' }}
                </p>
            </div>
            <div>
                <p class="text-xs text-slate-400 uppercase font-bold tracking-wider">Nomor SPK</p>
                <p class="text-sm font-mono text-slate-600">
                    {{ $alokasi->spk->nomor_spk ?? ($alokasi->id_spk ?? 'Tidak Terhubung') }}
                </p>
            </div>
        </div>
        ...
        <div class="grid grid-cols-2 gap-4">
            ...
            @if(isset($alokasi->kegiatan->mata_anggaran))
            <div>
        ...
        <div>
            ...
            <p class="text-sm text-slate-700 leading-relaxed">{{ $alokasi->kegiatan->nama_kegiatan ?? 'Detail Kegiatan Tidak Tersedia' }}</p>
        </div>
        ...
        <div class="grid grid-cols-2 gap-4 pt-2">
            <div>
                ...
                <div class="mt-1">
                    <p class="text-sm font-semibold text-slate-800">
                        Rp {{ number_format(($alokasi->volume_target ?? 0) * ($alokasi->harga_satuan_aktual ?? 0), 0, ',', '.') }}
                    </p>
                    <p class="text-[10px] text-slate-400 font-medium">
                        {{ number_format($alokasi->volume_target ?? 0, 2, ',', '.') }} x {{ number_format($alokasi->harga_satuan_aktual ?? 0, 0, ',', '.') }}
                    </p>
                </div>
            </div>
            <div>
        ...
        <div class="bg-blue-600 p-4 rounded-xl shadow-lg shadow-blue-100 mt-4">
            <div class="flex justify-between items-center text-white">
                <div class="flex flex-col">
                    ...
                    <span class="text-xl font-black tracking-tight">Rp {{ number_format($alokasi->total_honor ?? 0, 0, ',', '.') }}</span>
                </div>
