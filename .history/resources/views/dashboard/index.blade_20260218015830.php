        ...
        <div class="bg-white rounded-2xl p-6 shadow-sm border border-slate-100">
            <div class="flex justify-between items-start">
                <div>
                    ...
                    <p class="text-[10px] text-slate-400 mt-1">Sisa: {{ $total_spk - $spk_terarsip }} Dokumen</p>
                </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-slate-100 p-6">
            ...
            <div class="mt-6 space-y-2">
                <div class="flex justify-between text-xs">
                    <span class="text-slate-400">Sudah Scan</span>
                    <span class="font-bold text-slate-700">{{ $spk_terarsip }}</span>
                </div>
                <div class="flex justify-between text-xs">
                    <span class="text-slate-400">Belum Scan</span>
                    <span class="font-bold text-red-500">{{ $total_spk - $spk_terarsip }}</span>
                </div>
            </div>
        </div>
    </div>
                ...
                <div class="flex items-start gap-4 p-4 bg-blue-50 rounded-2xl border border-blue-100">
                    ...
                    <div>
                        ...
                        <p class="text-[11px] text-blue-700 leading-relaxed mt-1">
                            Dashboard menggunakan data <b>Bulan {{ \Carbon\Carbon::create()->month($bulan)->locale('id')->translatedFormat('F') }} {{ $tahun }}</b>. 
...
<script>
    ...
    new Chart(ctxBar, {
        type: 'bar',
        data: {
            labels: {!! json_encode($top_mitra_data->map(fn($m) => explode(' ', $m->mitra->nama_lengkap)[0] ?? 'N/A')) !!},
            datasets: [{
