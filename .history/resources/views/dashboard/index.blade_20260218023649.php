...
<div class="space-y-6">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 pb-10">
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <!-- Fitur Baru: Pencarian Cepat Kuota SBML Mitra (Sudah Diperbaiki) -->
            <div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden p-6">
                <div class="flex items-center gap-3 mb-6">
                    ...
                    <div>
                        ...
                        <p class="text-[10px] text-slate-400">Pilih mitra dari database untuk melihat pemakaian bulan {{ \Carbon\Carbon::create()->month((int)$bulan)->locale('id')->translatedFormat('F') }}.</p>
                    </div>
                </div>

                <div class="space-y-4">
                    <div class="relative">
                        <!-- PERBAIKAN: Mengambil dari seluruh daftar mitra -->
                        <select id="quickCheckMitra" class="w-full rounded-xl border-slate-200 py-3 px-4 text-sm font-medium focus:ring-2 focus:ring-indigo-500 transition-all cursor-pointer">
                            <option value="">-- Cari Nama Mitra di Database --</option>
                            @foreach($all_mitra_list as $m)
                                <option value="{{ $m->id }}" data-nama="{{ $m->nama_lengkap }}">{{ $m->nama_lengkap }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div id="quickResult" class="hidden animate-fade-in space-y-4 pt-4 border-t border-slate-50">
                        ...
                        <div class="p-4 bg-slate-50 rounded-xl border border-slate-100">
                            ...
                            <div class="w-full bg-slate-200 rounded-full h-2 overflow-hidden">
                                <div id="resBar" class="h-full transition-all duration-1000" style="width: 0%"></div>
                            </div>
                        </div>
                    </div>
                    
                    <div id="quickEmpty" class="text-center py-4 bg-slate-50 rounded-xl border border-dashed border-slate-200">
                        <p class="text-[11px] text-slate-400 italic">Silakan pilih nama mitra dari daftar di atas untuk melihat statusnya.</p>
                    </div>
                </div>
            </div>
        </div>
...
<script>
    ...
    // Logic Fitur Cek Cepat Mitra
    const selectQuick = document.getElementById('quickCheckMitra');
    const quickResult = document.getElementById('quickResult');
    const quickEmpty = document.getElementById('quickEmpty');

    selectQuick.addEventListener('change', function() {
        const mitraId = this.value;
        if (mitraId === "") {
            quickResult.classList.add('hidden');
            quickEmpty.classList.remove('hidden');
            return;
        }
        ...
        // Cek apakah mitra ada di map statistik (artinya punya SPK aktif)
        const data = mitraStatsMap[mitraId];

        if (data) {
            // Mitra Aktif
            document.getElementById('resKategori').innerText = data.kategori;
            document.getElementById('resSisa').innerText = 'Rp' + new Intl.NumberFormat('id-ID').format(data.sisa_rp);
            document.getElementById('resPersen').innerText = data.persen + '%';
            
            const bar = document.getElementById('resBar');
            bar.style.width = Math.min(100, data.persen) + '%';
            
            // Color logic
            bar.classList.remove('bg-emerald-500', 'bg-amber-500', 'bg-red-500');
            if(data.persen >= 90) bar.classList.add('bg-red-500');
            else if(data.persen >= 70) bar.classList.add('bg-amber-500');
            else bar.classList.add('bg-emerald-500');
        } else {
            // Mitra Inaktif (Belum ada SPK di bulan terpilih)
            document.getElementById('resKategori').innerText = "Belum Ada SPK Aktif";
            document.getElementById('resSisa').innerText = "Rp 0 (Plafon Belum Diatur)";
            document.getElementById('resPersen').innerText = '0%';
